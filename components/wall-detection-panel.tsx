    ...
    <div className="space-y-3">
      ...
      {metrics ? (
        ...
        <dl className="grid grid-cols-2 gap-x-3 gap-y-1 rounded-md bg-[#f6f7f8] p-3 text-xs text-[#5f6670]">
          ...
          {process.env.NODE_ENV !== "production" ? <><div><dt>Quality Score</dt><dd className="font-semibold text-[#202124]">{Math.round(metrics.averageQualityScore)}/100</dd></div><div><dt>Pipeline</dt><dd className="font-semibold text-[#202124]">{metrics.pipelineVersion}</dd></div></> : null}
      ...
      {process.env.NODE_ENV !== "production" && debugRegions.length ? (
        <details className="rounded-md border border-dashed border-[#cbd2da] p-2 text-xs text-[#5f6670]">
          ...
          {debugRegions.map((region) => <div key={region.id} className="mt-3"><p className="mb-1">{region.id} · confianza {Math.round(region.confidence * 100)}% · calidad {Math.round(region.qualityScore)}/100 · {region.refined.length} puntos · {region.componentCount} componentes · contorno {region.contour.length} puntos · reintentos {region.retryCount}</p><div className="grid grid-cols-2 gap-2">{(["original", "cleaned", "corrected", "final"] as const).map((stage) => <figure key={stage}><DebugPreview region={region} stage={stage} /><figcaption className="mt-0.5 text-center">{{ original: "Entrada binaria", cleaned: "Después de limpieza", corrected: "Después de corrección", final: "Máscara final" }[stage]}</figcaption></figure>)}</div><p className="mt-2 leading-5">Parámetros: suavizado {maskSmoothness.toFixed(2)} · polígono {polygonTolerance.toFixed(2)} px.</p><p className="leading-5">Etapas: {region.appliedStages.join(", ")}.</p><p className="leading-5">Scores: {Object.entries(region.qualityBreakdown).map(([name, score]) => `${name} ${(score * 100).toFixed(0)}%`).join(" · ")}.</p><p className="leading-5">Tiempos: {Object.entries(region.stageTimings).map(([name, time]) => `${name} ${time} ms`).join(" · ") || "sin etapas"}.</p>{region.issues.length ? <p className="leading-5 text-[#9a5b22]">Alertas: {region.issues.map((item) => item.type).join(", ")}.</p> : null}</div>)}
        </details>
      ) : null}
    </div>
  );
